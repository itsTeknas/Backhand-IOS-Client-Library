...
-(NSURLSessionTask*) suggestClubAdditionPostWithClubName: (NSString*) clubName
    ...
    completionHandler: (void (^)(NSObject* output, NSError* error)) handler {
    // verify the required parameter 'clubName' is set
    if (clubName == nil) {
        NSParameterAssert(clubName);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"clubName"] };
            NSError* error = [NSError errorWithDomain:kSWGUserApiErrorDomain code:kSWGUserApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    // verify the required parameter 'clubAddress' is set
    if (clubAddress == nil) {
        NSParameterAssert(clubAddress);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"clubAddress"] };
            NSError* error = [NSError errorWithDomain:kSWGUserApiErrorDomain code:kSWGUserApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/suggest_club_addition"];
    ...
    NSMutableDictionary* queryParams = [[NSMutableDictionary alloc] init];
    if (clubName != nil) {
        queryParams[@"club_name"] = clubName;
    }
    if (clubAddress != nil) {
        queryParams[@"club_address"] = clubAddress;
    }
    NSMutableDictionary* headerParams = [NSMutableDictionary dictionaryWithDictionary:self.apiClient.configuration.defaultHeaders];
    [headerParams addEntriesFromDictionary:self.defaultHeaders];
    // HTTP header `Accept`
